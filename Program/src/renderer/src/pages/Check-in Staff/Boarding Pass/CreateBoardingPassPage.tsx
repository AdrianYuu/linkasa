...
import { createBoardingPass } from '@renderer/controllers/BoardingPassController'
import {
  addPassengerToFlightSchedule,
  getFlightSchedules
} from '@renderer/controllers/FlightScheduleController'

const CreateBoardingPassPage = () => {
  ...
  useEffect(() => {
    ...
    if (!luggagePhoto) setErrorLuggagePhoto("Luggage's photo can't be empty!")
  }, [luggagePhoto])
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!luggagePhoto) return

    const fileName = Date.now() + '_' + luggagePhoto.name
    const storageRef = ref(storage, 'luggagesPhoto/' + fileName)
    ...
    await createBoardingPass(newBoardingPass)
    await addPassengerToFlightSchedule(flightID, newPassenger)
    resetData()
...
export default CreateBoardingPassPage
